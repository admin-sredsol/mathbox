 ...
 */
import type { Color, Matrix3, Matrix4, Quaternion, Vector2, Vector3, Vector4 } from "three";
import type { MathboxNode, MathboxSelection } from "../../types";
type OnInvalid = () => void;
type Validate<In, Out> = (value: In, target: unknown, invalid: OnInvalid) => Out;
export interface Type<In, Out> {
    validate: Validate<In, Out>;
    make(): Out;
}
export type Optional<T> = T | null | undefined;
export type BlendingModes = "no" | "normal" | "add" | "subtract" | "multiply" | "custom";
export type Axes = "x" | "y" | "z" | "w" | "W" | "H" | "D" | "I" | "width" | "height" | "depth" | "items" | 1 | 2 | 3 | 4;
...
export type TypeGenerators = {
    ...
    ivec4(x?: number, y?: number, z?: number, w?: number): Type<Vec4Like, Vector4>;
    mat3(n11?: number, n12?: number, n13?: number, n21?: number, n22?: number, n23?: number, n31?: number, n32?: number, n33?: number): Type<number[] | Matrix3, Matrix3>;
    mat4(n11?: number, n12?: number, n13?: number, n14?: number, n21?: number, n22?: number, n23?: number, n24?: number, n31?: number, n32?: number, n33?: number, n34?: number, n41?: number, n42?: number, n43?: number, n44?: number): Type<number[] | Matrix4, Matrix4>;
    quat(x?: number, y?: number, z?: number, w?: number): Type<Vec4Like | Quaternion, Quaternion>;
    color(r?: number, g?: number, b?: number, a?: number): Type<ColorDescription, Color>;
...
export type ArrayEmitter = (emit: Emit, i: number, t: number, delta: number) => void;
export type IntervalEmitter = (emit: Emit, x: number, i: number, t: number, delta: number) => void;
export type MatrixEmitter = (emit: Emit, i: number, j: number, t: number, delta: number) => void;
export type AreaEmitter = (emit: Emit, x: number, y: number, i: number, j: number, t: number, delta: number) => void;
export type VoxelEmitter = (emit: Emit, i: number, j: number, k: number, t: number, delta: number) => void;
export type VolumeEmitter = (emit: Emit, x: number, y: number, z: number, i: number, j: number, k: number, t: number, delta: number) => void;
export {};
